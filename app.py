...
from src.agents.energy_agent import SimpleEnergyAgent
from src.data_access import dbx
from src.charts.utils import (
    line_consumption,
    area_stacked,
    bar_top_regions,
    donut_voltage,   # voltage donut
)
...
# -------------------- Chat (simple + reliable) --------------------
st.markdown('<div class="card">', unsafe_allow_html=True)
st.subheader("Chat with your data")

if "simple_agent" not in st.session_state:
    st.session_state["simple_agent"] = SimpleEnergyAgent()
...
user_msg = st.chat_input("Ask e.g. 'Total kWh for Yorkshire last 7 days' or 'Predict next month for London'")
if user_msg:
    st.session_state["chat_history"].append(("user", user_msg))
    st.chat_message("user").markdown(user_msg)

    try:
        reply = st.session_state["simple_agent"].answer(user_msg, start_s, end_s, region)
...
st.caption("Auto schema discovery • Friendly region names from Gold • Filters drive every chart • Voltage donut • Heatmap removed • Chat answers totals/forecast/peak queries")
